...
AddEventHandler("GSR:TestCallback", function(tester, result)
    if result then
        TriggerClientEvent("GSR:Not1fy", tester, "~w~Subject Tested ~r~Positive ~w~for ~b~gun shot residue")
    else
        TriggerClientEvent("GSR:Not1fy", tester, "~w~Subject Tested ~g~Negative ~w~for ~b~gun shot residue")
    end
end)

RegisterNetEvent('GSR:CheckPerms')
AddEventHandler("GSR:CheckPerms", function ()
    if IsPlayerAceAllowed(source, Config.Perms.forceperm) then
        return true;
    else 
        return false
    end
...
function sendToDisc(title, message)
    ...
    embed = {
        {
            ...
            ["description"] = "" .. message ..  "",
            ["footer"] = {
                ["text"] = 'GSR | Created By Jordan.#2139 & Astra#2100 For CSRP',
            },
        }
    }
    -- Start
    ...
    function(err, text, headers) end, 'POST', json.encode({username = 'Rocket GSR Test', embeds = embed}), { ['Content-Type'] = 'application/json' })
  -- END
end
